...
# create lower R^2 with week auxiliary variables (r1 = 0.3)
...
capture.output(round(RBtable,3), file = 'pps_r.txt')

library("xtable")
RBtable = as.data.frame(RBtable)
Latex = xtable(RBtable, digits = c(0,c(rep(2,14))), caption = 'Estimates under pps with lower R^2')
Latex
